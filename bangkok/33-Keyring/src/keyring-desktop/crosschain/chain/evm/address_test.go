...
func (s *CrosschainTestSuite) TestGetAddressFromPublicKey() {
	...
	address, _, err := builder.GetAddressFromPublicKey(bytes)
	require.NoError(err)
	...
	address, _, err = builder.GetAddressFromPublicKey(bytes_compressed)
	require.NoError(err)
...
func (s *CrosschainTestSuite) TestGetAddressFromPublicKeyErr() {
	...
	address, _, err := builder.GetAddressFromPublicKey([]byte{})
	require.Equal(xc.Address(""), address)
	require.EqualError(err, "invalid secp256k1 public key")

	address, _, err = builder.GetAddressFromPublicKey([]byte{1, 2, 3})
	require.Equal(xc.Address(""), address)
	require.EqualError(err, "invalid secp256k1 public key")
}
