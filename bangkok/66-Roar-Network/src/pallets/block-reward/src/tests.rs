...
impl Rewards {
	...
	///
	/// Both current `total_issuance` and `TVL` are used. If these are changed after calling this
	/// function, they won't be reflected in the struct.
...
fn adjust_tvl_percentage(desired_tvl_percentage: Perbill) {
	...
	// Sanity check
	assert_eq!(<TestRuntime as Config>::Currency::total_issuance(), required_total_issuance);
}
