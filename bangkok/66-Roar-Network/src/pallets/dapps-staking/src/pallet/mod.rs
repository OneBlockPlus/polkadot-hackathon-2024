...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// Max number of unique `EraStake` values that can exist for a `(staker, contract)`
		...
		/// `EraStake` value by calling `bond&stake` or `unbond&unstake` if they've reached the max
		/// number of values.
		///
		...
		/// Number of eras that need to pass until dApp rewards for the unregistered contracts can
		/// be burned. Developer can still claim rewards after this period has passed, iff it hasn't
		/// been burned yet.
		///
		/// For example, if retention is set to `2` and current era is `10`, it means that all
		/// unclaimed rewards bellow era `8` can be burned.
		#[pallet::constant]
	...
	/// Simple map where smart contract points to basic info about it (e.g. developer address,
	/// state)
	#[pallet::storage]
	...
	pub enum Event<T: Config> {
		...
		NominationTransfer(T::AccountId, T::SmartContract, BalanceOf<T>, T::SmartContract),
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// Depending on the pallet configuration/state it is possible that developer needs to be
		/// whitelisted prior to registration.
		///
		...
		/// Unregister existing contract from dapps staking, making it ineligible for rewards from
		/// current era onwards. This must be called by the root (at the moment).
		///
		/// Deposit is returned to the developer but existing stakers should manually call
		/// `withdraw_from_unregistered` if they wish to to unstake.
		///
		...
		pub fn unregister(
			...
			ensure!(dapp_info.state == DAppState::Registered, Error::<T>::NotOperatedContract);
		...
		///
		/// Funds don't need to undergo the unbonding period - they are returned immediately to the
		/// staker's free balance.
		#[pallet::call_index(2)]
		...
		pub fn bond_and_stake(
			...
			Self::deposit_event(Event::<T>::BondAndStake(staker, contract_id, value_to_stake));
			Ok(().into())
		}
		...
		pub fn nomination_transfer(
			...
			ensure!(Self::is_active(&origin_contract_id), Error::<T>::NotOperatedContract);
			ensure!(Self::is_active(&target_contract_id), Error::<T>::NotOperatedContract);
		...
		// TODO: do we need to add force methods or at least methods that allow others to claim for
		// someone else?
		...
		///
		/// The rewards are always added to the staker's free balance (account) but depending on the
		/// reward destination configuration, they might be immediately re-staked.
		#[pallet::call_index(7)]
		...
		///
		/// Call must ensure that the specified era is eligible for reward payout and that it hasn't
		/// already been paid out for the dapp.
		#[pallet::call_index(8)]
		...
		pub fn maintenance_mode(
			...
			ensure!(is_disabled ^ enable_maintenance, Error::<T>::NoMaintenanceModeChange);
		...
		/// The purpose of this call is only for fixing one of the issues detected with
		/// dapps-staking.
		///
		/// The dispatch origin must be Root.
		...
		pub fn burn_stale_reward(
			...
			let dapp_info =
				RegisteredDapps::<T>::get(&contract_id).ok_or(Error::<T>::NotOperatedContract)?;
			ensure!(dapp_info.is_unregistered(), Error::<T>::NotUnregisteredContract);
			...
			// mark entry as `claimed` but it means it's just handled (want to avoid rename since
			// pallet will soon be redesigned).
			contract_stake_info.contract_reward_claimed = true;
			ContractEraStake::<T>::insert(&contract_id, era, contract_stake_info);
	...
	impl<T: Config> Pallet<T> {
		...
		fn calculate_dapp_reward(
			...
			ensure!(contract_stake_info.total > Zero::zero(), Error::<T>::NotStakedContract,);
		...
		///
		/// `StakerInfo` and `ContractStakeInfo` are provided and all checks are made to ensure that
		...
		/// unstaked value is returned. If not, an error is returned and structs are left in an
		/// undefined state.
		...
		///
		/// Note that this can differ from _total staked value_ since some funds might be undergoing
