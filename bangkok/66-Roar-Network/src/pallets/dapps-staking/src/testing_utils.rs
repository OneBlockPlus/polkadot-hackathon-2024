...
pub(crate) fn assert_register(developer: AccountId, contract_id: &MockSmartContract<AccountId>) {
	...
	assert_ok!(DappsStaking::register(RuntimeOrigin::root(), developer, contract_id.clone()));
	...
	assert_eq!(*contract_id, RegisteredDevelopers::<TestRuntime>::get(developer).unwrap());
...
pub(crate) fn assert_unregister(developer: AccountId, contract_id: &MockSmartContract<AccountId>) {
	...
	assert_ok!(DappsStaking::unregister(RuntimeOrigin::root(), contract_id.clone()));
	...
	assert_eq!(final_state.contract_info.total, init_state.contract_info.total);
	assert_eq!(
		final_state.contract_info.number_of_stakers,
		init_state.contract_info.number_of_stakers
	);

	assert_eq!(final_state.dapp_info.state, DAppState::Unregistered(current_era));
...
pub(crate) fn assert_withdraw_from_unregistered(
	...
	assert_eq!(init_state.era_info.staked, final_state.era_info.staked + staked_value);
	assert_eq!(init_state.era_info.locked, final_state.era_info.locked + staked_value);
	assert_eq!(init_state.dapp_info, final_state.dapp_info);
	assert_eq!(init_state.ledger.locked, final_state.ledger.locked + staked_value);
	assert_eq!(init_state.ledger.unbonding_info, final_state.ledger.unbonding_info);

	assert!(final_state.staker_info.latest_staked_value().is_zero());
	assert!(!GeneralStakerInfo::<TestRuntime>::contains_key(&staker, contract_id));
}

/// Perform `bond_and_stake` with all the accompanied checks including before/after storage
/// comparison.
pub(crate) fn assert_bond_and_stake(
	...
	assert_eq!(final_state.era_info.locked, init_state.era_info.locked + staking_value);
	assert_eq!(final_state.contract_info.total, init_state.contract_info.total + staking_value);
	assert_eq!(
		final_state.staker_info.latest_staked_value(),
		init_state.staker_info.latest_staked_value() + staking_value
	);
	assert_eq!(final_state.ledger.locked, init_state.ledger.locked + staking_value);
}
...
pub(crate) fn assert_unbond_and_unstake(
	...
	let remaining_staked = init_state.staker_info.latest_staked_value().saturating_sub(value);
	...
	// Ensure that total staked amount has been decreased for contract and staking points are
	// updated
	assert_eq!(
	...
	assert_eq!(init_state.era_info.staked - expected_unbond_amount, final_state.era_info.staked);
...
pub(crate) fn assert_withdraw_unbonded(staker: AccountId) {
	...
	assert_eq!(final_rewards_and_stakes.locked, init_era_info.locked - expected_unbond_amount);
	assert_eq!(final_ledger.locked, init_ledger.locked - expected_unbond_amount);
}
...
pub(crate) fn assert_nomination_transfer(
	...
	let expected_transfer_amount =
		if init_staked_value - value >= MINIMUM_STAKING_AMOUNT { value } else { init_staked_value };
	...
	// Ensure number of contracts has been increased on target contract it is first stake by the
	// staker
	let no_init_stake_on_target_contract =
		target_init_state.staker_info.latest_staked_value().is_zero();
	...
	if fully_unstaked_and_nothing_to_claim {
		assert!(!GeneralStakerInfo::<TestRuntime>::contains_key(&staker, &origin_contract_id));
	}
}
...
pub(crate) fn assert_claim_staker(claimer: AccountId, contract_id: &MockSmartContract<AccountId>) {
	...
	assert_ok!(DappsStaking::claim_staker(RuntimeOrigin::signed(claimer), contract_id.clone(),));
	...
	assert_restake_reward(&init_state_current_era, &final_state_current_era, calculated_reward);
	...
	if final_state_current_era.staker_info.is_empty() {
		...
		assert!(!GeneralStakerInfo::<TestRuntime>::contains_key(&claimer, contract_id));
	} else {
		assert!(new_era > claim_era);
	}
	assert!(new_era.is_zero() || new_era > claim_era);

	// Claim shouldn't mint new tokens, instead it should just transfer from the dapps staking
	// pallet account
	...
	assert_eq!(init_state_claim_era.contract_info, final_state_claim_era.contract_info);
}
...
fn assert_restake_reward(
	...
	if DappsStaking::should_restake_reward(
		...
		assert_eq!(init_state_current_era.era_info.staked, final_state_current_era.era_info.staked);
		assert_eq!(init_state_current_era.era_info.locked, final_state_current_era.era_info.locked);
		assert_eq!(init_state_current_era.contract_info, final_state_current_era.contract_info);
	}
}
...
pub(crate) fn assert_burn_stale_reward(
	...
	assert!(final_state.contract_info.contract_reward_claimed);
	assert_eq!(issuance_before_claim - calculated_reward, issuance_after_claim);
}
