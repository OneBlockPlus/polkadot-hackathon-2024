      ...
      <div className="flex max-w-[22rem] grow flex-col gap-4">
        ...
        <Card>
          <CardContent className="pb-3 pt-6">
            ...
            {Object.entries(chainInfo || {}).map(([key, value]) => (
              <div key={key} className="text-sm leading-7">
                {key}:
                <strong
                  className="float-right ml-6 max-w-[15rem] truncate"
                  title={value}
                >
                  {value}
                </strong>
          ...
          <CardFooter className="flex items-center justify-center space-x-3">
            {/* Explorer Link */}
            {!!activeChain?.explorerUrls &&
              !!Object.keys(activeChain.explorerUrls)?.length && (
                <Link
        ...
        {!activeChain?.testnet && (
          <>
            <h2 className="text-center font-mono text-red-400">
              Security Disclaimer
            </h2>
