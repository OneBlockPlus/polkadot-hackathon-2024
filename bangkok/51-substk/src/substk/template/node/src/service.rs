...
use substk_client_kv_store_service::{KvServiceParams, SubstrateConfig};
...
pub fn new_full<N: sc_network::NetworkBackend<Block, <Block as sp_runtime::traits::Block>::Hash>>(
    ...
    kv_service_params: KvServiceParams,
    ...
    let kv_cfg = kv_service_params.create_configuration(&config);
    ...
    // after chain jsonrpc server.
    if let Some(kv_cfg) = kv_cfg {
        kv_cfg.run_axum::<SubstrateConfig>(Arc::new(task_manager.spawn_handle()));
    }
