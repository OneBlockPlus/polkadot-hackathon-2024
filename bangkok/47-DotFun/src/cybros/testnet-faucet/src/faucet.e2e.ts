...
import { getLatestMessage, postMessage } from "src/test/matrixHelpers";
import { destroyDataSource, E2ESetup, getDataSource, setup, teardown } from "src/test/setupE2E";
...
describe("Faucet E2E", () => {
  ...
  test("Faucet drips to a user that has requested a drip 30h ago", async () => {
    ...
    oldDrip.timestamp = new Date(Date.now() - 30 * 60 * 60 * 1000).getTime();
  ...
  test("Web faucet doesn't drip to address that has requested a drip 5h ago", async () => {
    ...
    const oldDrip = new Drip();
    oldDrip.addressSha256 = sha256(userAddress);
    oldDrip.timestamp = new Date(Date.now() - 5 * 60 * 60 * 1000).getTime();
