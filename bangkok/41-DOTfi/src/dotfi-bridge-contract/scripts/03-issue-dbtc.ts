...
async function main() {
  ...
  const tokenName = "DOTfi Bitcoin";
  const tokenSymbol = "dBTC";
  const tokenDecimals = 18;
  const maxSupply = ethers.parseUnits("21000000", tokenDecimals);

  try {
    console.log(`Issuing new token ${tokenName} with max supply of 21 million...`);
