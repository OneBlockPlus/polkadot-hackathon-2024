...
export class Vnft {
  ...
  public async admins(originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<Array<ActorId>> {
    ...
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId || ZERO_ADDRESS,
  ...
  public async drawCard(originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<string> {
    const payload = this._program.registry.createType('(String, String)', ['Vnft', 'DrawCard']).toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId || ZERO_ADDRESS,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
      payload,
  ...
  public async tokenId(originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<bigint> {
  ...
  public async tokenMetadataById(token_id: number | string | bigint, originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<TokenMetadata | null> {
    const payload = this._program.registry.createType('(String, String, U256)', ['Vnft', 'TokenMetadataById', token_id]).toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId || ZERO_ADDRESS,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
      payload,
  ...
  public async balanceOf(owner: ActorId, originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<bigint> {
    const payload = this._program.registry.createType('(String, String, [u8;32])', ['Vnft', 'BalanceOf', owner]).toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId || ZERO_ADDRESS,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
      payload,
  ...
  public async getApproved(token_id: number | string | bigint, originAddress?: string, value?: number | string | bigint, atBlock?: `0x${string}`): Promise<ActorId> {
    const payload = this._program.registry.createType('(String, String, U256)', ['Vnft', 'GetApproved', token_id]).toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId || ZERO_ADDRESS,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
      payload,
  ...
  public subscribeToMintedEvent(callback: (data: { to: ActorId; token_metadata: TokenMetadata }) => void | Promise<void>): Promise<() => void> {
    return this._program.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {;
      ...
      if (getServiceNamePrefix(payload) === 'Vnft' && getFnNamePrefix(payload) === 'Minted') {
        callback(this._program.registry.createType('(String, String, {"to":"[u8;32]","token_metadata":"TokenMetadata"})', message.payload)[2].toJSON() as unknown as { to: ActorId; token_metadata: TokenMetadata });
      }
    });
  }

  public subscribeToBurnedEvent(callback: (data: { from: ActorId; token_id: number | string | bigint }) => void | Promise<void>): Promise<() => void> {
    return this._program.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {;
      if (!message.source.eq(this._program.programId) || !message.destination.eq(ZERO_ADDRESS)) {
        return;
      }
      ...
      if (getServiceNamePrefix(payload) === 'Vnft' && getFnNamePrefix(payload) === 'Burned') {
        callback(this._program.registry.createType('(String, String, {"from":"[u8;32]","token_id":"U256"})', message.payload)[2].toJSON() as unknown as { from: ActorId; token_id: number | string | bigint });
      }
    });
  }
