...
---
QuickStart
# Start the Anvil blockchain simulator and set the block time to 5 seconds
anvil --block-time 5
# Start Anvil with a fork of the Moonbeam network's RPC URL, simulating blockchain, and set block time to 5 seconds
anvil --fork-url https://rpc.api.moonbase.moonbeam.network --block-time 5
# Load environment variables from the .env file
source .env
# Use Forge tool to run the smart contract deployment script, specify local RPC URL and private key, deploy the contract, and broadcast the transaction
...
# Navigate to the front-end project directory NoSandwichSwapPair_Web
cd Web/NoSandwichSwapPair_Web
# Run the npm development server to start the front-end project
npm run dev
# Use Forge to run unit tests, matching the test cases named "testTradeDex1" or "testTradeDex2", and display logs in verbose mode
