...
if [ "$RA_METHOD" = "dcap" ]; then
  if [ -e "/opt/pflix/releases/current/dcap-ver" ]; then
    echo "Dcap version found"
    rm -rf /opt/pflix/releases/current/dcap-ver/data
    mv /opt/pflix/releases/current/dcap-ver/* /opt/pflix/releases/current/
    rm -rf /opt/pflix/releases/current/dcap-ver
  fi
else
  if [ -e "/opt/pflix/releases/current/epid-ver" ]; then
    echo "Epid version found"
    rm -rf /opt/pflix/releases/current/epid-ver/data
    mv /opt/pflix/releases/current/epid-ver/* /opt/pflix/releases/current/
    rm -rf /opt/pflix/releases/current/epid-ver
  fi
fi
...
echo "Starting Pflix with extra opts '${EXTRA_OPTS}'"
if [ "$SGX" -eq 0 ]; then
  echo "Pflix will running in software mode"
  cd $WORK_DIR && $GRAMINE_DIRECT_BIN pflix $EXTRA_OPTS
else
  echo "Pflix will running in hardware mode"
  cd $WORK_DIR && $GRAMINE_SGX_BIN pflix $EXTRA_OPTS
fi
