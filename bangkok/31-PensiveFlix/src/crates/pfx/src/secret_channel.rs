use pfx_api::crypto::EncryptedData;
...
mod sender {
    ...
    use pfx_api::crypto::{ecdh, EncryptedData};
    ...
    impl<'a, MsgChan: MessageChannel> pfx_mq::traits::MessageChannel for BoundSecretMessageChannel<'a, MsgChan> {
    ...
    impl<'a, MsgChan: MessagePrepareChannel> pfx_mq::traits::MessagePrepareChannel
        for BoundSecretMessageChannel<'a, MsgChan>
    {
        type Signer = MsgChan::Signer;

        fn prepare_with_data(&self, data: Vec<u8>, to: impl Into<Path>) -> pfx_mq::SigningMessage<Self::Signer> {
            let payload = self.encrypt_payload(data);
...
mod receiver {
    use super::Payload;
    use anyhow::bail;
    use pfx_mq::{MessageOrigin, ReceiveError, TypedReceiver};
    use pfx_api::crypto::ecdh;
            ...
            let msg = match self.peeler.peel(msg) {
                ...
                Err(PeelError::CodecError) =>
                    if origin.always_well_formed() {
                        panic!("Failed to decode critical mq message, please upgrade the pflix client");
                    } else {
