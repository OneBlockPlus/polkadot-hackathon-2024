...
#[clap(about = "The PFLX TEE worker app.", version = VERSION, author)]
struct Args {
    ...
    /// Remove corrupted checkpoint so that pflix can restart to continue to load others.
    #[arg(long)]
    ...
    /// Handover key from another running pflix instance
    #[arg(long)]
    ...
    /// Disable the RCU policy to update the Pflix state.
    #[arg(long)]
    no_rcu: bool,
    ...
    /// Custom pflix data directory in non-SGX environment
    #[arg(long)]
...
enum Commands {
    /// Show Pflix version details
    Version,
    /// Show Pflix target information
    TargetInfo,
}
...
async fn serve(sgx: bool, args: Args) -> Result<()> {
    ...
    run_pflix_server(init_args, GraminePlatform, listener_addr).await?;
