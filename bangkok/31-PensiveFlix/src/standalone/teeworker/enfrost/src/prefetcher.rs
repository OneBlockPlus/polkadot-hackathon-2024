use anyhow::Result;
use pfx_api::blocks::{BlockHeader, BlockHeaderWithChanges};
use pfx_xt::{BlockNumber, ChainApi};
    ...
    let storage_changes = crate::chain_client::fetch_storage_changes(client, &from_hash, &to_hash)
        ...
        .map(|(offset, storage_changes)| {
            BlockHeaderWithChanges {
                // Headers are synced separately. Only the `number` is used in Pflix while syncing blocks.
