...
async fn try_load_handover_proof(pr: &mut PflixClient, chain_api: &ChainApi) -> Result<()> {
    ...
    let proof = chain_client::read_proofs(
        ...
        Some(hash),
        vec![
            &storage_key("TeeWorker", "PflixBinAddedAt")[..],
            &storage_key("TeeWorker", "PflixBinAllowList")[..],
...
async fn try_register_worker(
    ...
    if let Some(attestation) = info.attestation {
        ...
        info!("No attestation evidence from pflix runtime_info!");
        Ok(false)
    }
}
...
async fn try_load_chain_state(
    ...
    let Ok(pubkey) = hex::decode(pubkey) else {
        return Err(anyhow!("Pflix returned an invalid pubkey"));
    };
        ...
        None => {
            ...
            if let Some(endpoint) = args.public_endpoint.clone() {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        error!("call pflix.set_endpoint() response error: {:?}", e);
                        return Ok(false);
                    }
                }
            } else {
...
async fn bridge(
    ...
    let mut cc = PflixClient::connect(args.internal_endpoint.clone()).await?;
    ...
    // Try to initialize Pflix and register on-chain
    ...
    if !args.no_init {
        if !info.initialized {
            info!("Pflix not initialized. Requesting init...");
            ...
            info!("Pflix already initialized.");
        }
    ...
    if args.no_sync {
        ...
        if args.public_endpoint.is_some() && info.public_key().is_some() {
            // Here the reason we dont directly report errors when `try_update_worker_endpoint` fails is that we want the endpoint can be registered anytime (e.g. days after the enfrost initialization)
    ...
    info!("Start updating pflix's ra report task...");
    ...
    loop {
        // update the latest Pflix state
        let info = cc.get_info(()).await?.into_inner();
        info!("Pflix get_info response: {:#?}", info);
        ...
        // send the blocks to Pflix in batch
        ...
        // check if Pflix has already reached the chain tip.
        if synced_blocks == 0 && !more_blocks {
            ...
            // Launch key handover if required only when the old Pflix is up-to-date
            if args.next_pflix_endpoint.is_some() {
                let mut next_pr =
                    PflixClient::connect(args.next_pflix_endpoint.clone().unwrap()).await?;
...
async fn handover_worker_key(server: &mut PflixClient, client: &mut PflixClient) -> Result<()> {
...
async fn batch_sync_block(
    ...
    macro_rules! sync_blocks_to {
        ($to: expr) => {
            if next_blocknum <= $to {
                batch_sync_storage_changes(pflix_api, chain_api, next_blocknum, $to, batch_window)
                    .await?;
