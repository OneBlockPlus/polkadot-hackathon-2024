...
use pfx_node_primitives::{opaque::Block, AccountId, Balance, BlockNumber, Hash, Nonce};
...
where
	...
	C::Api: BabeApi<Block>,
	C::Api: pfx_pallet_mq_runtime_api::MqApi<Block>,
...
{
	use pfx_node_rpc_ext::{NodeRpcExt, NodeRpcExtApiServer};
	...
	io.merge(NodeRpcExt::new(client, backend, pool).into_rpc())
		.expect("Initialize PFLX node RPC ext failed.");
