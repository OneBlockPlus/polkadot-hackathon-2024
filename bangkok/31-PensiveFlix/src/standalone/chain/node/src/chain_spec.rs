use pfx_node_primitives::{AccountId, Balance, Block, Signature};
use pfx_node_runtime::{constants::currency::DOLLARS, wasm_binary_unwrap, MaxNominations, SessionKeys, StakerStatus};
...
pub use pfx_node_runtime::RuntimeGenesisConfig;
...
fn properties() -> Properties {
	...
	properties.insert("tokenSymbol".into(), "PFLX".into());
	properties.insert("tokenDecimals".into(), 18.into());
	properties.insert("ss58Format".into(), 5168.into());
	properties
}

fn pfx_testnet_genesis() -> serde_json::Value {
	#[rustfmt::skip]
...
fn pfx_devnet_config_genesis() -> serde_json::Value {
	#[rustfmt::skip]
...
pub fn pfx_testnet_config() -> ChainSpec {
	ChainSpec::from_json_bytes(&include_bytes!("../res/pfx-testnet-spec-raw.json")[..]).unwrap()
}

pub fn pfx_devnet_config() -> ChainSpec {
	ChainSpec::from_json_bytes(&include_bytes!("../res/pfx-devnet-spec-raw.json")[..]).unwrap()
}

pub fn pfx_devnet_generate_config() -> ChainSpec {
	ChainSpec::builder(wasm_binary_unwrap(), Default::default())
		.with_name("pfx-devnet")
		.with_id("pfx-devnet")
		.with_chain_type(ChainType::Live)
		.with_properties(properties())
		.with_genesis_config_patch(pfx_devnet_config_genesis())
...
pub fn pfx_testnet() -> ChainSpec {
	ChainSpec::builder(wasm_binary_unwrap(), Default::default())
		.with_name("pfx-testnet")
		.with_id("pfx-testnet")
		.with_chain_type(ChainType::Live)
		.with_properties(properties())
		.with_genesis_config_patch(pfx_testnet_genesis())
		.with_telemetry_endpoints(
			TelemetryEndpoints::new(vec![(STAGING_TELEMETRY_URL.to_string(), 0)])
				.expect("Staging telemetry url is valid; qed"),
		)
...
fn testnet_genesis(
	...
	serde_json::json!({
		...
		"babe": {
			"epochConfig": Some(pfx_node_runtime::BABE_GENESIS_EPOCH_CONFIG)
		},
		"evmChainId": {
			"chainId": 5168
		},
