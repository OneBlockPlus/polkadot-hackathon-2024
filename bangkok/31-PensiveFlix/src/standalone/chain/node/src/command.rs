...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		let spec = match id {
			"" | "pfx-testnet" => Box::new(chain_spec::pfx_testnet_config()),
			"pfx-initial-devnet" => Box::new(chain_spec::pfx_devnet_generate_config()),
			"pfx-devnet" => Box::new(chain_spec::pfx_devnet_config()),
			"pfx-initial-testnet" => Box::new(chain_spec::pfx_testnet()),
...
pub fn run() -> sc_cli::Result<()> {
	...
	match &cli.subcommand {
		...
		Some(Subcommand::Benchmark(cmd)) => {
			...
			use pfx_node_runtime::ExistentialDeposit;
