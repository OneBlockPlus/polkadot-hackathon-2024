use pfx_types::messaging::SignedMessage;
...
pub fn register_worker(pflix_info: Vec<u8>, attestation: Vec<u8>, v2: bool) -> EncodedPayload {
    let call_name = if v2 {
        "register_worker_v2"
    } else {
        "register_worker"
    ...
    EncodedPayload::new(
        "TeeWorker",
        call_name,
        (Encoded(pflix_info), Encoded(attestation)).encode(),
    )
}

pub fn refresh_tee_status(pflix_info: Vec<u8>, attestation: Vec<u8>) -> EncodedPayload {
    EncodedPayload::new(
        "TeeWorker",
        "refresh_tee_status",
        (Encoded(pflix_info), Encoded(attestation)).encode(),
