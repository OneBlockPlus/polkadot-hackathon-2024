...
pub use pfx_api::{crpc::SystemInfo, ecall_args::InitArgs};
use core::fmt;
pub use master_key::{persistence::is_master_key_exists_on_local, PflixMasterKey};
use pfx_pallet_tee_worker::MasterKeySubmission;
use parity_scale_codec::{Decode, Encode};
use pfx_crypto::{ecdh::EcdhKey, key_share, rsa::RsaDer, sr25519::KDF, SecretKey};
use pfx_mq::{
...
impl<Platform: pal::Platform> System<Platform> {
    ...
    pub fn process_next_message(&mut self, block: &mut BlockDispatchContext) -> anyhow::Result<bool> {
        let ok = pfx_mq::select_ignore_errors! {
    ...
    fn init_keyfairy(&mut self, block: &mut BlockDispatchContext, master_key: PflixMasterKey) {
    ...
    fn process_master_key_launch_reqeust(
        ...
        if is_master_key_exists_on_local(&self.sealing_path) {
            ...
            let master_key =
                PflixMasterKey::from_sealed_file(&self.sealing_path, &self.identity_key.0, &self.platform)
                    .expect("PflixMasterKey from sealed file");
        ...
        let new_master_key = PflixMasterKey::generate().expect("PflixMasterKey generate");
    ...
    ///
    /// This function could panic a lot, thus should only handle data from other pflixs.
    ...
    fn process_master_key_distribution(
        ...
        let master_key = PflixMasterKey::from_rsa_der(&rsa_der)
            .map_err(|e| anyhow!("failed build PflixMasterKey from rsa_der: {e}"))?;
