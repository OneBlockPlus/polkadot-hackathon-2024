...
fn generate_rsa_private_key(bit_size: usize) -> Result<RsaPrivateKey, Error> {
    let mut rng = OsRng;
    let rsa_priv_key = RsaPrivateKey::new(&mut rng, bit_size).map_err(|e| Error::RsaError(format!("{:?}", e)))?;
    Ok(rsa_priv_key)
}
...
pub mod persistence {
    ...
    use pfx_types::{wrap_content_to_sign, SignedContentType};
    ...
    pub fn seal(sealing_path: &str, rsa_key_der: RsaDer, identity_key: &sr25519::Pair, sealer: &impl Sealing) {
        ...
        // TODO: seal with identity key so the newly handovered pflix do not need to do an extra sync to get master
        // key
