use pfx_mq::traits::MessageChannel;
use pfx_mq::MessageOrigin;
...
fn test_send_message() {
    ...
    use pfx_mq::{MessageSendQueue, MessageSigner};
...

    {
        ...
        handle00.push_data(b"payload00".to_vec(), b"pflx.network/test0".to_vec());

        let signer = TestSigner(b"key1".to_vec());

        let handle01 = queue.channel(runtime.clone(), signer);
        handle01.push_data(b"payload01".to_vec(), b"pflx.network/test1".to_vec());

        handle00.push_data(b"payload02".to_vec(), b"pflx.network/test1".to_vec());
...
#[test]
fn test_dispatcher() {
    use pfx_mq::{Message, MessageDispatcher};
