...
pub fn validate_ias_report(
	...
	// Validate Pflix
	let pflix_hash = sgx_fields.measurement_hash();
	if verify_pflix_hash && !pflix_bin_allowlist.contains(&pflix_hash) {
...
pub fn validate_dcap(
	...
	// Validate report
	let (report_data, pflix_hash, tcb_status, advisory_ids) = sgx_attestation::dcap::verify::verify(
		quote,
		collateral,
		now,
	...
	// Validate Pflix
	if verify_pflix_hash && !pflix_bin_allowlist.contains(&fixed_measurement_hash(&pflix_hash)) {
		return Err(Error::PflixRejected);
	}
	...
	Ok(ConfidentialReport {
		...
		measurement_hash: fixed_measurement_hash(&pflix_hash),
		confidence_level
	})
}
