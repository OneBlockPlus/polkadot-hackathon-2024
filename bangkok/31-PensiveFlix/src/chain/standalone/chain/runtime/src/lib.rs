...
pub use pfx_node_primitives::{AccountId, AccountIndex, Balance, BlockNumber, Hash, Moment, Nonce, Signature};
...
use frame_support::{
	...
	traits::{
		...
		EqualPrivilegeOnly, Imbalance, InstanceFilter, KeyOwnerProofSystem, LinearStoragePrice, LockIdentifier, Nothing, OnFinalize,
		OnUnbalanced,
	},
	weights::{
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("gringotts-node"),
	impl_name: create_runtime_str!("gringotts-node"),
	authoring_version: 1,
	...
	RuntimeCall: From<LocalCall>,
{
	fn create_transaction<C: frame_system::offchain::AppCrypto<Self::Public, Self::Signature>>(
		...
		let extra = (
			...
			pfx_pallet_mq::CheckMqSequence::<Runtime>::new(),
...
mod runtime {
	...
	pub type CesMq = pfx_pallet_mq;

	#[runtime::pallet_index(103)]
	pub type TeeWorker = pfx_pallet_tee_worker;

	#[runtime::pallet_index(104)]
	pub type EvmAccountMapping = pfx_pallet_evm_account_mapping;
	//------------------- PFLX's end ---------------------
}
...
pub type SignedExtra = (
	...
	pfx_pallet_mq::CheckMqSequence<Runtime>,
...
impl pfx_pallet_mq::CallMatcher<Runtime> for MqCallMatcher {
	fn match_call(call: &RuntimeCall) -> Option<&pfx_pallet_mq::Call<Runtime>> {
		match call {
...
impl pfx_pallet_mq::Config for Runtime {
	type QueueNotifyConfig = msg_routing::MessageRouteConfig;
	type CallMatcher = MqCallMatcher;
	type MasterPubkeySupplier = pfx_pallet_tee_worker::Pallet<Runtime>;
}
...
parameter_types! {
	...
	pub const VerifyPflix: bool = if cfg!(not(feature = "verify-pflixbin")) { false } else { true };
	pub const AtLeastWorkBlock: BlockNumber = DAYS / 2;
}

impl pfx_pallet_tee_worker::Config for Runtime {
	type Currency = Balances;
	type RuntimeEvent = RuntimeEvent;
	type TeeWorkerPalletId = TeeWorkerPalletId;
	type SchedulerMaximum = SchedulerMaximum;
	type WeightInfo = pfx_pallet_tee_worker::weights::SubstrateWeight<Runtime>;
	type MaxWhitelist = MaxWhitelist;
	type AtLeastWorkBlock = AtLeastWorkBlock;
	type LegacyAttestationValidator = pfx_pallet_tee_worker::IasValidator;
	type NoneAttestationEnabled = NoneAttestationEnabled;
	type VerifyPflix = VerifyPflix;
...
parameter_types! {
	...
	pub EIP712ChainID: pfx_pallet_evm_account_mapping::EIP712ChainID = sp_core::U256::from(0);
	pub EIP712VerifyingContractAddress: pfx_pallet_evm_account_mapping::EIP712VerifyingContractAddress = sp_core::H160::from([0u8; 20]);
}

impl pfx_pallet_evm_account_mapping::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type RuntimeCall = RuntimeCall;
	type Currency = Balances;
	type AddressConverter = pfx_pallet_evm_account_mapping::SubstrateAddressConverter;
	...
	type WeightInfo = pfx_pallet_evm_account_mapping::weights::SubstrateWeight<Runtime>;
}
//------------------------- PFLX's end -------------------------
...
impl_runtime_apis! {
	...
	//------------------------- PFLX's begin -------------------------
	impl pfx_pallet_mq_runtime_api::MqApi<Block> for Runtime {
		fn sender_sequence(sender: &pfx_types::messaging::MessageOrigin) -> Option<u64> {
