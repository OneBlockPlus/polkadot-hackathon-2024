...
use crate::{
    chain_client,
    types::{PflixClient, SrSigner},
    Args,
};
use anyhow::{anyhow, Context, Result};
use pfx_types::attestation::legacy::Attestation;
use pfx_api::crpc;
use pfx_xt::{
...
async fn apply_master_key(
    ...
    let tx_params = Params::new()
        ...
        .build();
    let tx = pfx_xt::dynamic::tx::apply_master_key(encoded_payload, signature);
...
pub async fn update_worker_endpoint(
    ...
    let tx_params = Params::new()
        ...
        .mortal(latest_block.header(), args.longevity)
        .build();
    let tx = pfx_xt::dynamic::tx::update_worker_endpoint(encoded_endpoint_payload, signature);
    let ret = para_api
        .tx()
        .create_signed_with_nonce(&tx, &signer.signer, signer.nonce(), tx_params)?
        .submit_and_watch()
...
pub async fn register_worker(
    ...
    let tx = pfx_xt::dynamic::tx::register_worker(encoded_runtime_info, attestation, v2);
...
pub async fn update_worker_ra_report(
    ...
    debug!("encoded attestation: {}", hex::encode(&attestation));
    let tx = pfx_xt::dynamic::tx::refresh_tee_status(encoded_runtime_info, attestation);
