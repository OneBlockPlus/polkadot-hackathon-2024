...
pub use pfx_xt::rpc::{self, StorageData, StorageKey};
pub use pfx_xt::{self, subxt};
use sp_consensus_grandpa::SetId;
pub use subxt::backend::legacy::rpc_methods::NumberOrHex;

pub type PflixClient = pflix_client::PflixClient<Channel>;
pub type SrSigner = pfx_xt::PairSigner;
...
#[derive(Parser, Debug)]
#[command(
    about = "Sync messages between pflix and the blockchain.",
    version,
    author
)]
pub struct Args {
    ...
    #[arg(short = 'n', long = "no-init", help = "Should init Pflix?")]
    pub no_init: bool,

    #[arg(
        long = "no-sync",
        help = "Don't sync Pflix. Quit right after initialization."
    )]
    pub no_sync: bool,

    #[arg(long, help = "Don't write Pflix egress data back to Substarte.")]
    pub no_msg_submit: bool,

    #[arg(
        long,
        help = "Inject dev key (0x1) to Pflix. Cannot be used with remote attestation enabled."
    )]
    ...
    #[arg(
        ...
        long,
        help = "The http endpoint to access Pflix's internal service"
    )]
    pub internal_endpoint: String,

    #[arg(
        long,
        help = "The http endpoint where Pflix provides services to the outside world"
    )]
    pub public_endpoint: Option<String>,

    #[arg(
        long,
        help = "Pflix http endpoint to handover the key. The handover will only happen when the old Pflix is synced."
    )]
    pub next_pflix_endpoint: Option<String>,
    ...
    #[arg(
        default_value = "4",
        long = "sync-blocks",
        help = "The batch size to sync blocks to Pflix."
    )]
    ...
    /// Take checkpoint from Pflix after completing the first block synchronization in Enfrost
    #[arg(long)]
...
pub mod utils {
    ...
    use pfx_xt::subxt::backend::legacy::rpc_methods::ReadProof;
