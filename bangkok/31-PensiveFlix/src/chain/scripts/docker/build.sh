...
function usage() {
    cat <<EOF
Easy to build PFLX docker images

Usage:
    $0 [options]

Options:
    -b <program name>  which program image to build, options: node pflix and enfrost
    ...
    -v <enable 'verify-pflixbin' feature to build>  options: 1(default) or 0
    -m <method of pflix attestation> options:dcap or epid(default)
    -l <docker build runtime log print out> options: 1 or 0(default)
    -c <pflix build version>  8-digit integer, date +%y%m%d%H for default value
...
function build_pflix() {
    ...
    if [[ -z $PFLIX_VERSION ]]; then
        PFLIX_VERSION=$(date +%y%m%d%H)
    fi
    docker_build_args+=(
        --build-arg PFLIX_VERSION=$PFLIX_VERSION
    )
    echo "SGX_ENV: $SGX_ENV"
    echo "IAS_API_KEY: $IAS_API_KEY"
    echo "IAS_SPID: $IAS_SPID"
    echo "PFLIX_VERSION: $PFLIX_VERSION"
    echo "RA_METHOD: $RA_METHOD"
    local docker_file="$the_script_dir/pflix/gramine/handover.Dockerfile"
    image_id="$org_id/pflix:$image_tag"
    echo "begin build image $image_id ..."
...
function build_enfrost() {
    local docker_file="$the_script_dir/enfrost/Dockerfile"
    image_id="pflixlab/enfrost:$image_tag"
    echo "begin build image $image_id ..."
    local progress_option
    progress_option=$(print_docker_build_log)
    docker_build $progress_option -t $image_id -f $docker_file ${docker_build_args[@]} $docker_build_ctx_dir
...
while getopts ":hpn:b:x:t:s:o:v:m:l:c:" opt; do
    ...
    s)
        ...
        ;;
    b)
        if [[ $OPTARG != "pflix" && $OPTARG != "enfrost" && $OPTARG != "node" ]]; then
            echo "Invalid program name: $OPTARG, options: pflix enfrost or node"
            exit 1
        fi
