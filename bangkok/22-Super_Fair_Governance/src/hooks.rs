...
#[cfg_attr(
    all(not(feature = "tuples-96"), not(feature = "tuples-128")),
    impl_for_tuples(64)
)]
#[cfg_attr(
    all(feature = "tuples-96", not(feature = "tuples-128")),
    impl_for_tuples(96)
)]
...
mod tests {
    ...
    fn on_initialize_and_on_runtime_upgrade_weight_merge_works() {
        ...
        TestExternalities::default().execute_with(|| {
            ...
            assert_eq!(
                <(Test, Test)>::on_runtime_upgrade(),
                Weight::from_parts(40, 0)
            );
        });
    }
    ...
    fn on_idle_round_robin_works() {
        ...
        unsafe {
            ...
            assert_eq!(
                ON_IDLE_INVOCATION_ORDER,
                ["Test1", "Test2", "Test3"].to_vec()
            );
