...
export class DiffManager {
  ...
  public async acceptSolution(message: ClientMessage) {
    ...
    if (this.originalUri && this.modifiedUri) {
      await deleteFileIfExists(this.originalUri);
      await deleteFileIfExists(this.modifiedUri);
    }
  }
}

async function deleteFileIfExists(uri: vscode.Uri) {
  try {
    await vscode.workspace.fs.stat(uri); // Check if the file exists
    await vscode.workspace.fs.delete(uri); // Delete the file if it exists
  } catch (error) {
    console.error(`Failed to delete file: ${uri.toString()}`);
  }
}
