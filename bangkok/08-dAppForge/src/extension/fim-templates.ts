...
export const getFimPromptTemplateDappForge = ({
  prefixSuffix
}: FimPromptTemplate) => {
...
function getFimTemplateAuto(fimModel: string, args: FimPromptTemplate) {
  ...
  if (fimModel.includes(FIM_TEMPLATE_FORMAT.dappforge)) {
    return getFimPromptTemplateDappForge(args)
  }
...
function getFimTemplateChosen(format: string, args: FimPromptTemplate) {
  ...
  if (format == FIM_TEMPLATE_FORMAT.dappforge) {
    return getFimPromptTemplateDappForge(args)
  }
...
export const getStopWordsChosen = (format: string) => {
  ...
  if (format === FIM_TEMPLATE_FORMAT.dappforge) return ['']
