...
async function main() {
    ...
    const evmContractInstance = new ethers.Contract(evmContractAddress, abi, wallet);
    console.log('evm contract instance created');

    let callPara = [
        `{"VM":"wasm", "Account":"`,
        wasmContractAddress,
        `","Fun":"echo","InputType":["string","vec","u8","u8","u8"],"InputValue":["`,
        `test string!","3","231","19","6"],"OutputType":[["string","vec"],["2"],["u8"]]}`,
    ].join('');
    console.log('Alith is calling echo method of wasm contract via evm contract, and the input parameters are: \n', callPara);
    const tx = await evmContractInstance.evmCallWasmProxy.staticCall(callPara);
    console.log('Transaction info:', tx);
    console.log('Alith has successfully called echo method of wasm contract via evm contract and got the echo result');
    process.exit(0);
}
