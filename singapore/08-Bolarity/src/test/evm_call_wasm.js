...
async function main() {
    ...
    const tx = await evmContractInstance.evmCallWasm(transferTo, amount, wasmContractAddress);
    await tx.wait();
    console.log('Transaction info:', tx);
    console.log(`Alith has successfully transferred ${amount / BigInt(10 ** 18)} WTT (wasm token) to Baltathar via evm contract call`);
    process.exit(0);
}
