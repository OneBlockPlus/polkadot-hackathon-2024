...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		KeysRegistered { nimbus_id: NimbusId, account_id: T::AccountId, keys: T::Keys },
		/// An NimbusId has been de-registered, and its AccountId mapping removed.
		KeysRemoved { nimbus_id: NimbusId, account_id: T::AccountId, keys: T::Keys },
		/// An NimbusId has been registered, replacing a previous registration and its mapping.
		KeysRotated { new_nimbus_id: NimbusId, account_id: T::AccountId, new_keys: T::Keys },
	}
	...
	impl<T: Config> Pallet<T> {
		...
		fn register_keys(
			...
			<Pallet<T>>::deposit_event(Event::KeysRegistered { nimbus_id, account_id, keys });
			Ok(())
		}
		fn rotate_keys(
			old_nimbus_id: NimbusId,
			...
			let new_stored_info = RegistrationInfo { keys: new_keys.clone(), ..stored_info };
			MappingWithDeposit::<T>::insert(&new_nimbus_id, &new_stored_info);
			NimbusLookup::<T>::insert(&account_id, &new_nimbus_id);

			<Pallet<T>>::deposit_event(Event::KeysRotated { new_nimbus_id, account_id, new_keys });
			Ok(())
		}
		pub fn enact_registration(
			...
			let info = RegistrationInfo { account: account_id.clone(), deposit, keys };
	...
	impl<T: Config> KeysLookup<NimbusId, T::Keys> for Pallet<T> {
		...
		fn set_keys(id: NimbusId, account: T::AccountId, keys: T::Keys) {
			...
			MappingWithDeposit::<T>::insert(
				id,
				RegistrationInfo { account, deposit: Zero::zero(), keys },
			);
		}
	}
