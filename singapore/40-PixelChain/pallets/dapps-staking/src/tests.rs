...
fn rewards_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_eq!(BlockRewardAccumulator::<TestRuntime>::get(), Default::default());
		...
		DappsStaking::rewards(Balances::issue(stakers_reward), Balances::issue(dapps_reward));
...
fn on_initialize_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		for era in 1..current_era {
			...
			assert_eq!(get_total_reward_per_era(), reward_info.stakers + reward_info.dapps);
		}
...
fn new_era_is_handled_with_maintenance_mode() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_eq!(DappsStaking::next_era_starting_block(), (5 * mock::BLOCKS_PER_ERA));
	})
}
...
fn new_era_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_eq!(joint_block_reward(), block_reward.stakers + block_reward.dapps);
		...
		assert_eq!(expected_era_reward, era_rewards.rewards.dapps + era_rewards.rewards.stakers);
...
fn general_staker_info_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		for era in starting_era..mid_era {
			...
			assert!(!ContractEraStake::<TestRuntime>::contains_key(&second_contract_id, era));
		}
...
fn bond_and_stake_different_value_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		// Bond&stake more than what's available in funds. Verify that only what's available is
		// bonded&staked.
		let staker_id = 2;
		...
		// Try to bond&stake more than we have available (since we already locked most of the free
		// balance).
...
fn bond_and_stake_insufficient_value() {
	ExternalityBuilder::build().execute_with(|| {
		...
		// If user tries to make an initial bond&stake with less than minimum amount, raise an
		// error.
		assert_noop!(
			DappsStaking::bond_and_stake(
				RuntimeOrigin::signed(staker_id),
		...
		// Now try to bond&stake some additional funds and expect an error since we cannot
		// bond&stake 0.
		assert_noop!(
...
fn unbond_and_unstake_too_many_unlocking_chunks_is_not_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		// Ensure that we're at the max but can still add new chunks since it should be merged with
		// the existing one
		assert_eq!(MAX_UNLOCKING_CHUNKS, DappsStaking::ledger(&staker).unbonding_info.len());
		assert_unbond_and_unstake(staker, &contract_id, unstake_amount);
...
fn withdraw_unbonded_full_vector_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		// This is a sanity check for the test. Some chunks should remain, otherwise test isn't
		// testing realistic unbonding period.
		assert!(!Ledger::<TestRuntime>::get(&staker_id).unbonding_info.is_empty());

		while !Ledger::<TestRuntime>::get(&staker_id).unbonding_info.is_empty() {
			advance_to_era(DappsStaking::current_era() + 1);
			assert_withdraw_unbonded(staker_id);
		}
	})
...
fn nomination_transfer_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert!(!GeneralStakerInfo::<TestRuntime>::get(&staker, &origin_contract_id)
			.latest_staked_value()
			.is_zero());

		// The second operation should fully unstake origin contract since it takes it below minimum
		// staking amount
		assert_nomination_transfer(
			staker,
			&origin_contract_id,
...
fn nomination_transfer_contracts_have_too_many_era_stake_values() {
	ExternalityBuilder::build().execute_with(|| {
		...
		// Ensure it's not possible to transfer from origin contract since it's era stake values are
		// maxed
		assert_noop!(
			DappsStaking::nomination_transfer(
				RuntimeOrigin::signed(staker),
		...
		// Swap origin and target to verify that same is true if target contract era stake values is
		// maxed out
...
fn nomination_transfer_max_number_of_stakers_exceeded() {
	ExternalityBuilder::build().execute_with(|| {
		...
		// Fill up the second contract with stakers until max number of stakers limit has been
		// reached
...
fn claiming_when_stakes_full_without_compounding_is_ok() {
	ExternalityBuilder::build().execute_with(|| {
		...
		// claim and restake once, so there's a claim record for the for the current era in the
		// stakes vec
		assert_claim_staker(staker_id, &contract_id);

		// making another gap in eras and trying to claim and restake would exceed
		// MAX_ERA_STAKE_VALUES
		advance_to_era(DappsStaking::current_era() + 1);
		assert_noop!(
...
fn dev_stakers_split_util() {
	...
	let era_info = EraInfo::<Balance> {
		rewards: RewardInfo { dapps: base_dapps_reward, stakers: base_stakers_reward },
		staked: total_staked,
		locked: total_staked,
	};
	...
	assert_eq!(calculated_stakers_reward + calculated_dev_reward, dev_reward + stakers_reward);
}
...
fn custom_max_encoded_len() {
	...
	assert_eq!(UnbondingInfo::<u128>::max_encoded_len(), max_unbonding_info_len as usize);

	let max_staker_info_len = 10 * (4 + 16) + 1;
	assert_eq!(StakerInfo::<u128>::max_encoded_len(), max_staker_info_len as usize);
}
