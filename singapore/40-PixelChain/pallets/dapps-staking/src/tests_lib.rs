...
fn unbonding_info_test() {
	...
	for x in unbonding_info.len() as usize..chunks.len() {
		...
		assert!(unbonding_info.vec().windows(2).all(|w| w[0].unlock_era <= w[1].unlock_era));
	}
...
fn staker_info_stake_ops() {
	...
	assert_eq!(staker_info.latest_staked_value(), first_stake + second_stake);
	...
	assert_eq!(staker_info.latest_staked_value(), first_stake + second_stake + third_stake);
...
fn staker_info_unstake_ops() {
	...
	assert_eq!(staker_info.len(), 2);
	assert_eq!(staker_info.latest_staked_value(), total_staked - first_unstake);
	...
	assert_eq!(staker_info.len(), 3);
	assert_eq!(staker_info.latest_staked_value(), total_staked - second_unstake);
...
fn staker_info_claim_ops_advanced() {
	...
	// Second claim
	assert_eq!(staker_info.claim(), (second_stake_era, first_stake_value + second_stake_value));
	assert_eq!(staker_info.len(), 2);

	// Third claim, expect that 3rd era stake is the same as second
	assert_eq!(staker_info.claim(), (3, first_stake_value + second_stake_value));
	assert_eq!(staker_info.len(), 1);
