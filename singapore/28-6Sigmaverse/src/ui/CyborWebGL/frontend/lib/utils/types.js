...
const getJsTypeDef = (typeDef, payloadMethod) => {
    if (typeDef.isPrimitive) {
        ...
        if (primitive.isActorId)
            return { type: 'ActorId', imports: ['ActorId'] };
        if (primitive.isCodeId)
            return { type: 'CodeId', imports: ['CodeId'] };
        if (primitive.isMessageId)
            return { type: 'MessageId', imports: ['MessageId'] };
        if (primitive.isH256)
            return { type: 'H256', imports: ['H256'] };
        if (primitive.isH160)
            return { type: 'H160', imports: ['H160'] };
        if (primitive.isNonZeroU8)
            return { type: 'NonZeroU8', imports: ['NonZeroU8'] };
        if (primitive.isNonZeroU16)
            return { type: 'NonZeroU16', imports: ['NonZeroU16'] };
        if (primitive.isNonZeroU32)
            return { type: 'NonZeroU32', imports: ['NonZeroU32'] };
        if (primitive.isNonZeroU64)
            return { type: 'NonZeroU64', imports: ['NonZeroU64'] };
        if (primitive.isNonZeroU128)
            return { type: 'NonZeroU128', imports: ['NonZeroU128'] };
        if (primitive.isNonZeroU256)
            return { type: 'NonZeroU256', imports: ['NonZeroU256'] };
    }
...
export { getJsTypeDef, getScaleCodecDef };
