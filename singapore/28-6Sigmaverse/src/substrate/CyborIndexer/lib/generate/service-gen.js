import { getPayloadMethod, PayloadMethod } from '../utils/payload-method.js';
...
const createPayload = (serviceName, fnName, params) => {
    if (params.length === 0) {
...
class ServiceGenerator extends BaseGenerator {
    _program;
    scaleTypes;
    constructor(out, _program, scaleTypes) {
        super(out);
        this._program = _program;
        this.scaleTypes = scaleTypes;
    }
    generate(className = 'Program') {
        this._out
            ...
            for (const service of this._program.services) {
                this._out.line(`public readonly ${toLowerCaseFirst(service.name)}: ${service.name === className ? service.name + 'Service' : service.name}`);
            }
            this._out
                .line()
    ...
    generateProgramConstructor() {
        if (!this._program.ctor || this._program.ctor.funcs.length === 0)
            return;
        for (const { name, params } of this._program.ctor.funcs) {
            const args = this.getArgs(params);
            this._out
                .block(`${getFuncName(name)}CtorFromCode(code: Uint8Array | Buffer${args !== null ? ', ' + args : ''}): TransactionBuilder<null>`, () => {
                this._out
                    ...
                    .line(params.length === 0 ? `'${name}',` : `['${name}', ${params.map(({ name }) => name).join(', ')}],`, false)
                    .line(params.length === 0
                    ? `'String',`
                    : `'(String, ${params.map(({ def }) => getScaleCodecDef(def)).join(', ')})',`, false)
                    .line(`'String',`, false)
                    .line(`code,`, false)
                    .reduceIndent()
                    .line(`)`)
                ...
                .line()
                .block(`${getFuncName(name)}CtorFromCodeId(codeId: ${HEX_STRING_TYPE}${args !== null ? ', ' + args : ''})`, () => {
                this._out
                    .line(`const builder = new TransactionBuilder<null>(`, false)
                    .increaseIndent()
                    .line(`this.api,`, false)
            ...
            this._out.line().block(this.getFuncSignature(name, params, returnType, isQuery), () => {
                if (isQuery) {
                    this._out
                        ...
                        .reduceIndent()
                        .line(`})`)
                        .line("if (!reply.code.isSuccess) throw new Error(this._program.registry.createType('String', reply.payload).toString())")
                        .line(`const result = this._program.registry.createType('(String, String, ${returnScaleType})', reply.payload)`)
                    ...
                    this._out
                        ...
                        .line(params.length === 0
                        ? `['${service.name}', '${name}'],`
                        : `['${service.name}', '${name}', ${params.map(({ name }) => name).join(', ')}],`, false)
                        .line(params.length === 0
                        ? `'(String, String)',`
                        : `'(String, String, ${params.map(({ def }) => getScaleCodecDef(def)).join(', ')})',`, false)
    ...
    generateSubscriptions(service) {
        ...
        for (const event of service.events) {
            ...
            this._out
                .line()
                .block(`public subscribeTo${event.name}Event(callback: (data: ${jsType}) => void | Promise<void>): Promise<() => void>`, () => {
                this._out
                    .line(`return this._program.api.gearEvents.subscribeToGearEvent('UserMessageSent', ({ data: { message } }) => {`)
                    .increaseIndent()
                    .block(`if (!message.source.eq(this._program.programId) || !message.destination.eq(ZERO_ADDRESS))`, () => {
                    this._out.line(`return`);
                })
                    .line()
                    .line(`const payload = message.payload.toHex()`)
                    .block(`if (getServiceNamePrefix(payload) === '${service.name}' && getFnNamePrefix(payload) === '${event.name}')`, () => {
                    if (jsType === 'null') {
                        this._out.line(`callback(null)`);
                    }
                    else {
                        this._out.line(`callback(this._program.registry.createType('(String, String, ${getScaleCodecDef(event.def, true)})', message.payload)[2].${decodeMethod}() as unknown as ${jsType})`);
                    }
                })
                    .reduceIndent()
                    .line(`})`);
    ...
    getFuncSignature(name, params, returnType, isQuery) {
        const args = this.getArgs(params);
