...
```sh
cargo build --package cloak-node --release
```
...
```sh
./target/release/cloak-node --dev
```

To purge the development chain's state, run the following command:

```sh
./target/release/cloak-node purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/cloak-node -ldebug --dev
```
...
$ ./target/release/cloak-node --dev --base-path ./my-chain-state/
