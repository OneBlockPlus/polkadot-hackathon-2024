;(function(root){
	...
	var self=reg[me.funKey]={
		...
		setModule:function(mid,data){module[mid]=data},
		getModule:function(mid){return module[mid]==undefined?false:module[mid]},
		...
		/**3D显示入口方法部分**/
		autoShow:function(mid,target){
			self.domInit(target);		//1.初始化three.js的运行环境
			var md=self.getModule(mid);
			var x=parseFloat(md.x),y=parseFloat(md.y),z=parseFloat(md.z);
			self.struct(md,function(){
		...
		struct:function(obj,ck){
			...
			var bcfg={position:[0,0,0],scale:[1,1,1],rotation:[-Math.PI/2,0,0],userData:{}}
		...
		setCamera:function(x,y,z){
			...
			run.camera.position.set(x+10,y+10,z+10);		//2倍距离于物体
